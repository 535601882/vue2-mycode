...
(function () {
  ...
  logger.options.today = new Date().getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate();
  // 获取默认baseUrl
      ...
      if (!isElementTarget) {
        ...
        if (paths && paths.length > 0) {
          ...
          arr.forEach((item, index) => {
            let className = item.className ? "." + item.className.replace(/\s+/g, ".") : "";
          ...
          let className = currentNode.className ? "." + currentNode.className.replace(/\s+/g, ".") : "";
          if (tagName != "body") {
            selectorArr.push(tagName + className);
            currentNode = currentNode.parentNode;
            tagName = currentNode.tagName.toLowerCase();
            className = currentNode.className ? "." + currentNode.className.replace(/\s+/g, ".") : "";
            if (tagName != "body") {
              selectorArr.push(tagName + className);
              currentNode = currentNode.parentNode;
              tagName = currentNode.tagName.toLowerCase();
